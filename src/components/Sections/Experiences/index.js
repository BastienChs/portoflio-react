...
const Experiences = () => {
    ...
    const displayExperiences =  experiencesList.sort((a,b) =>b.start_date.localeCompare(a.start_date)).map((experience, index) => {
        ...
        return(
            <div key={index} className={'w-full grid grid-cols-6 p-2 experience-container rounded shadow-lg'}>
                <div className={'company-image-container self-center'}>
    ...
    return(
      <section className="experiences min-h-screen" id={"experiences-section"}>
          <main>
              <h2 className={'w-full text-left pl-10 text-3xl font-bold font-opensans underline uppercase text-gray-500 opacity-30 mt-6'}>03. My experiences</h2>
